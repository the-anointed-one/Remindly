...
export class PaystackProvider implements BillingProvider {
  ...
  async createCustomer(
    ...
    try {
      ...
      if (data.status) {
        this.logger.log(
          `Paystack customer created: ${data.data.customer_code}`,
        );
        return {
          success: true,
    ...
    try {
      ...
      const { data } = await axios.post(
        `${PAYSTACK_BASE}/transaction/initialize`,
        {
          ...
          amount: params.amount, // Added this field
          currency: this.configService.get('PAYSTACK_CURRENCY', 'USD'),
          callback_url: this.configService.get(
            'PAYSTACK_CALLBACK_URL',
            `${this.configService.get('FRONTEND_URL', 'http://localhost:3001')}/dashboard`,
          ),
          metadata: {
      ...
      if (data.status) {
        this.logger.log(
          `Paystack subscription initialized: ${data.data.reference}`,
        );
        return {
          success: true,
      ...
      this.logger.error(
        `Paystack initializeSubscription failed for plan ${params.planCode}: ${errorMessage}`,
      );
      return {
        success: false,
        error: errorMessage,
  ...
  async chargeAuthorization(
    ...
    try {
      ...
      return {
        success: false,
        error: data.data?.gateway_response || data.message,
      };
    } catch (error: any) {
      this.logger.error(
        `Paystack chargeAuthorization failed: ${error.message}`,
      );
      return {
        success: false,
        error: error.response?.data?.message || error.message,
