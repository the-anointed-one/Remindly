...
export class RateLimitService {
  ...
  constructor(
    ...
    this.hourlySmsLimit = this.configService.get<number>(
      'HOURLY_SMS_RATE_LIMIT',
      20,
    );
    this.dailyAiLimit = this.configService.get<number>(
  ...
  private enforceWindow(
    ...
    if (entry.count >= limit) {
      const resetIn = Math.ceil(
        (entry.windowStart + windowMs - now) / 1000 / 60,
      );
