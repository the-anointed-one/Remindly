...
export class AIService {
    ...
    private async logUsage(
        ...
        this.logger.log(`AI usage: tenant=${tenantId} feature=${feature} tokens=${tokensUsed}`);
    }
    ...
    async generateTemplate(
        tenantId: string,
        userId: string,
        params: { businessType: string; channel: string; purpose: string; tone?: string },
    ) {
...
Return ONLY the message text.`;
        ...
        const result = await this.openAIProvider.complete({ systemPrompt, userPrompt, maxTokens: 200, temperature: 0.8 });

        if (!result.success) throw new ForbiddenException(`AI generation failed: ${result.error}`);

        await this.logUsage(tenantId, 'generate_template', userPrompt, result.text!, result.tokensUsed || 0, result.model || 'unknown');

        await this.auditService.log({
            tenantId, userId, action: 'CREATE', entity: 'AIGeneration', entityId: 'generate_template',
...
Return ONLY the improved message text.`;
        ...
        const result = await this.openAIProvider.complete({ systemPrompt, userPrompt, maxTokens: 200, temperature: 0.7 });

        if (!result.success) throw new ForbiddenException(`AI improvement failed: ${result.error}`);

        await this.logUsage(tenantId, 'improve_template', userPrompt, result.text!, result.tokensUsed || 0, result.model || 'unknown');

        await this.auditService.log({
            tenantId, userId, action: 'CREATE', entity: 'AIGeneration', entityId: 'improve_template',
            newValues: { feature: 'improve_template', tokens: result.tokensUsed },
        });

        return { original: params.currentTemplate, improved: result.text, tokensUsed: result.tokensUsed };
    }
...
Return ONLY the rewritten message.`;
        ...
        if (!result.success) throw new ForbiddenException(`AI tone change failed: ${result.error}`);

        await this.logUsage(tenantId, 'change_tone', userPrompt, result.text!, result.tokensUsed || 0, result.model || 'unknown');

        await this.auditService.log({
            tenantId, userId, action: 'CREATE', entity: 'AIGeneration', entityId: 'change_tone',
            newValues: { feature: 'change_tone', tone: params.targetTone, tokens: result.tokensUsed },
        });

        return { original: params.currentTemplate, rewritten: result.text, tone: params.targetTone, tokensUsed: result.tokensUsed };
    }
    ...
    async optimizeConfirmationRate(
        tenantId: string,
        userId: string,
        params: { currentTemplate: string; channel: string; businessType?: string; currentConfirmationRate?: number },
    ) {
        await this.enforceAccess(tenantId);
...
Return ONLY the optimized message.`;
        ...
        const result = await this.openAIProvider.complete({ systemPrompt, userPrompt, maxTokens: 250, temperature: 0.6 });

        if (!result.success) throw new ForbiddenException(`AI optimization failed: ${result.error}`);

        await this.logUsage(tenantId, 'optimize_confirmation', userPrompt, result.text!, result.tokensUsed || 0, result.model || 'unknown');

        await this.auditService.log({
            tenantId, userId, action: 'CREATE', entity: 'AIGeneration', entityId: 'optimize_confirmation',
            newValues: { feature: 'optimize_confirmation', tokens: result.tokensUsed },
        });

        return { original: params.currentTemplate, optimized: result.text, tokensUsed: result.tokensUsed };
    }
    ...
    async getUsageStats(tenantId: string) {
        const tenant = await this.prisma.tenant.findUnique({
            where: { id: tenantId },
            select: { aiUsageCount: true, aiTrialLimit: true, aiMonthlyLimit: true, trialActive: true, planType: true },
        });

        if (!tenant) throw new ForbiddenException('Tenant not found');

        const limit = tenant.trialActive ? tenant.aiTrialLimit : tenant.aiMonthlyLimit;

        const recentLogs = await this.prisma.aIUsageLog.findMany({
            where: { tenantId },
            orderBy: { createdAt: 'desc' },
            take: 10,
            select: { feature: true, tokensUsed: true, model: true, costUsd: true, createdAt: true },
        });
